...
def stop_running_jobs(glue_client, job_name):
    """
    Stop any running instances of the job toavoid concurrent runs.
    
    Parameters:
        - glue_client (boto3.client): Glue client to use to interact with AWS Glue.
        - job_name (str): Name of the job to stop.
    """
    try:
        ...
        for job_run in response['JobRuns']:
            if job_run['JobRunState'] in ['RUNNING', 'STARTING']:
                glue_client.batch_stop_job_run(JobName=job_name, JobRunIds=[job_run['Id']])
                print(f"Stopping running instance of job: {job_run['Id']}")
                # Wait for the job to fully stop
                while True:
                    status = check_job_status(glue_client, job_name, job_run['Id'])
                    if status['JobRunState'] in ['STOPPED', 'FAILED', 'SUCCEEDED']:
                        break
                    print(f"Waiting for job {job_run['Id']} to stop. Current status: {status['JobRunState']}")
                    time.sleep(10)
    except ClientError as e:
...
def create_glue_job(
    ...
    **kwargs
    ...
    # Check and stop any running instances of the job
    stop_running_jobs(glue_client, job_name)
    ...
    if fm_credentials is not None:
        arguments['--fm_username'] = fm_credentials['--fm_username']
        arguments['--fm_password'] = fm_credentials['--fm_password']
    
    if year_quarter is not None:
        arguments['--year'] = year_quarter['--year']
        arguments['--last_quarter'] = year_quarter['--last_quarter']
